...
func main() {
	...
	if err != nil {
		setupLog.Error(err, "unable to set up spire OIDC Discovery Provider resource controller manager")
		os.Exit(1)
	}
	if err = spireOIDCDiscoveryProviderControllerManager.SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "failed to set spire OIDC Discovery Provider controller with manager",
			"controller", utils.ZeroTrustWorkloadIdentityManagerSpiffeCsiDriverControllerName, "manager")
		os.Exit(1)
	}
