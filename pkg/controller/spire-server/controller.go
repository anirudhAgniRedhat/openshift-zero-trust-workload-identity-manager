...
func New(mgr ctrl.Manager) (*SpireServerReconciler, error) {
	c, err := NewSpireServerControllerClient(mgr)
	if err != nil {
		return nil, err
	}
	return &SpireServerReconciler{
		SpireServerCtrlClient: c,
...
func (r *SpireServerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	err = r.Get(ctx, types.NamespacedName{Name: spireServerConfigMap.Name, Namespace: spireServerConfigMap.Namespace}, &existingSpireServerCM)
	if err != nil && kerrors.IsNotFound(err) {
		if err := r.Create(ctx, spireServerConfigMap); err != nil {
		...
		if err := r.Update(ctx, &existingSpireServerCM); err != nil {
	...
	err = r.Get(ctx, types.NamespacedName{Name: spireControllerManagerConfigMap.Name, Namespace: spireControllerManagerConfigMap.Namespace}, &existingSpireControllerManagerCM)
	if err != nil && kerrors.IsNotFound(err) {
		if err := r.Create(ctx, spireControllerManagerConfigMap); err != nil {
			return ctrl.Result{}, fmt.Errorf("failed to create ConfigMap: %w", err)
		}
		...
		if err := r.Update(ctx, &existingSpireControllerManagerCM); err != nil {
			return ctrl.Result{}, fmt.Errorf("failed to update ConfigMap: %w", err)
		}
		r.log.Info("Updated ConfigMap with new config")
	} else if err != nil {
	...
	err = r.Create(ctx, spireBundleCM)
