	...
	tests := []struct {
		...
		},
		{
			...
			expected: func(deployment *appsv1.Deployment) {
				...
				// Check the spiffe-oidc-discovery-provider container resources
				oidcContainer := containers[0]
				assert.Equal(t, "spiffe-oidc-discovery-provider", oidcContainer.Name)
				assert.Equal(t, resource.MustParse("100m"), oidcContainer.Resources.Requests[corev1.ResourceCPU])
				assert.Equal(t, resource.MustParse("128Mi"), oidcContainer.Resources.Requests[corev1.ResourceMemory])
				assert.Equal(t, resource.MustParse("500m"), oidcContainer.Resources.Limits[corev1.ResourceCPU])
				assert.Equal(t, resource.MustParse("512Mi"), oidcContainer.Resources.Limits[corev1.ResourceMemory])
			},
		},
	}
...
func TestGetResourceRequirements(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "nil input returns empty requirements",
			input:    nil,
			expected: corev1.ResourceRequirements{},
		},
		{
			name: "non-nil input returns same requirements",
			input: &corev1.ResourceRequirements{
				Requests: corev1.ResourceList{
					corev1.ResourceCPU:    resource.MustParse("100m"),
					corev1.ResourceMemory: resource.MustParse("128Mi"),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := getResourceRequirements(tt.input)
			assert.Equal(t, tt.expected, result)
		})
	}
}
...
func TestDerefTolerations(t *testing.T) {
	tests := []struct {
		...
		{
			name: "tolerations with nil entries",
			input: []*corev1.Toleration{
				{
					Key:      "node-role",
					Operator: corev1.TolerationOpEqual,
			...
			expected: []corev1.Toleration{
				...
				{}, // nil pointer becomes zero value
				{
					Key:      "dedicated",
					Operator: corev1.TolerationOpExists,
					Effect:   corev1.TaintEffectNoExecute,
