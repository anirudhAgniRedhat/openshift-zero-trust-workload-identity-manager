...
func (r *StaticResourceReconciler) CreateOrApplyRbacResources(ctx context.Context) error {
	...
	for _, clusterRole := range clusterRoles {
		err := r.CreateOrUpdateObject(ctx, clusterRole)
		if err != nil {
	...
	for _, clusterRoleBinding := range clusterRoleBindings {
		err := r.CreateOrUpdateObject(ctx, clusterRoleBinding)
		if err != nil {
	...
	for _, role := range roles {
		err := r.CreateOrUpdateObject(ctx, role)
		if err != nil {
	...
	for _, roleBinding := range roleBindings {
		err := r.CreateOrUpdateObject(ctx, roleBinding)
		if err != nil {
